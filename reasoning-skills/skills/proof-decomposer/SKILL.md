...
| `bin/proof-structure-check.py` | **mechanizes B1/B2** — reads a proof skeleton (premises/axioms/steps+`from`/goal); asserts no dangling citation, a DAG (no circular reasoning), and a reachable goal; flags off-path (irrelevant) steps. `<skeleton.json>` · `selftest` |
| `bin/numeric-spotcheck.py` | **mechanizes B3** — searches a finite integer sample space for a counterexample to a parametric claim via a **safe** AST evaluator (no `eval`); a counterexample is a disproof. `<claim.json>` · `selftest` |
