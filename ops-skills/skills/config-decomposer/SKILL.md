...
| `references/secrets-and-safety.md` | **the B4 safety review** — the mechanizable smell taxonomy (plaintext secrets, `:latest`/unpinned, `0.0.0.0/0`, wildcard grants, missing limits), the reference/placeholder guard, and least-privilege; mechanized by `bin/config-lint.py` |
...
| `bin/config-lint.py` | **mechanizes the B4 floor** — a static smell detector across YAML/JSON/HCL/Dockerfile/TOML: plaintext secrets · `:latest`/unpinned · `0.0.0.0/0` · wildcard grants · missing k8s limits; `<file\|dir>` · `selftest` |
