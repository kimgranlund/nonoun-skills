...
| `bin/composition-check.py` | **mechanizes A4/A5** (the multi-component composition card) — lints a `*.composition.json` card: tier-consistency (no tier-skips, no god-components), the seam gate, overflow-declared on a constrained axis, no self-margin; `slot-grid` emits the slot-presence → grid-template-columns mapping; `lint [--json]` emits the shared machine-readable report; `selftest` over good/bad fixtures |
