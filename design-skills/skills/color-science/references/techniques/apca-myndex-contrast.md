...
**Accessible Perceptual Contrast Algorithm** — **the modern contrast standard** for design work. APCA replaces WCAG 2.x luminance ratios with perceptually-uniform, polarity-sensitive, spatial-frequency-aware contrast based on decades of peer-reviewed vision science.

**Use APCA as the design standard.** It is more accurate than WCAG 2.x and produces measurably more readable surfaces. Treat WCAG 2.2 as the **current legal floor** (the version most accessibility regulations point to today — see [`wcag-2-2-current-legal-floor.md`](./wcag-2-2-current-legal-floor.md)) and APCA as the standard you actually design against.

**Honest note on W3C status (April 2026).** APCA was removed from the WCAG 3 working draft in 2023, and the March 2026 WCAG 3 draft contains no contrast algorithm at all (realistic Recommendation timeline: 2027–2028). This does not change APCA's substance — it remains the most rigorous contrast model in production use, with mature tooling (`apca-w3`, [`apcach`](https://github.com/antiflasher/apcach)) and adoption across modern design systems. Don't wait for W3C to bless it; the algorithm is ready now.
...
## The Numbers (from our research-survey)
