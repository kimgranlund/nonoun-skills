...
export function spectrumToKS(reflectance) {
    return reflectance.map(reflectanceToKS);
}
/** K/S spectrum → reflectance spectrum. */
export function KSToSpectrum(KS) {
 ...
 */
export function mix(spectrumA, spectrumB, concentrationA) {
 ...
 */
export function mixN(spectra, concentrations) {
    ...
    if (sum <= 0)
        throw new Error('concentrations must sum to > 0');
