...
function oklchToLinearRGB(oklch, toRGB) {
...
export function inGamut(rgb, epsilon = 0) {
    return (rgb[0] >= -epsilon && rgb[0] <= 1 + epsilon &&
 ...
 */
export function peakL(C, hDeg, toRGB, iterations = 32) {
    const h = wrapHueDeg(hDeg);
    let lo = 0;
    let hi = 1;
    for (let i = 0; i < iterations; i++) {
        const mid = (lo + hi) / 2;
        const rgb = oklchToLinearRGB([mid, C, h], toRGB);
        if (inGamut(rgb))
            lo = mid;
        else
            hi = mid;
 ...
 */
export function peakC(L, hDeg, toRGB, iterations = 32, cMax = 0.4) {
    const h = wrapHueDeg(hDeg);
    let lo = 0;
    let hi = cMax;
    for (let i = 0; i < iterations; i++) {
 ...
 */
export function peakLOverHue(C, toRGB, hueSteps = 720) {
    let bestL = 0;
    let bestH = 0;
...
export const peakL_sRGB = (C, hDeg) => peakL(C, hDeg, M_XYZ_TO_SRGB);
export const peakL_P3 = (C, hDeg) => peakL(C, hDeg, M_XYZ_TO_P3);
export const peakL_Rec2020 = (C, hDeg) => peakL(C, hDeg, M_XYZ_TO_REC2020);
export const peakC_sRGB = (L, hDeg) => peakC(L, hDeg, M_XYZ_TO_SRGB);
export const peakC_P3 = (L, hDeg) => peakC(L, hDeg, M_XYZ_TO_P3);
export const peakC_Rec2020 = (L, hDeg) => peakC(L, hDeg, M_XYZ_TO_REC2020, 32, 0.5);
