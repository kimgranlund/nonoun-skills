 ...
 */
export function mapToGamutOklch(origin, toRGB, inGamutFn) {
    const [L, C, hDeg] = origin;
    // Edge cases.
    if (inGamutFn(origin))
        return origin;
    if (L >= 1)
        return makeOklch(1, 0, hDeg);
    if (L <= 0)
        return makeOklch(0, 0, hDeg);
...
function clipFromOklch(c, toRGB) {
    ...
    if (toRGB === M_XYZ_TO_SRGB) {
        ...
        oklch = oklchSpace.fromXYZ(rec2020Space.toXYZ(clipped));
    }
    return oklch;
}
// ─── Gamut-specific convenience wrappers ──────────────────────────────────────
export function mapToSRGB(c) {
...
export function mapToRec2020(c) {
