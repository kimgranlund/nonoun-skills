...
| `bin/brand-spec-check.py` | **mechanizes the B axis** — `lint <card.brand.json>` (well-formed · UNTRACED · LOW_CONFIDENCE · COLLAPSED_TRUTH · CONTRAST_FAIL · BARE_TOKEN · GENERIC_IDEA · INCOMPLETE) · `contrast <fg> <bg> [large\|ui]` · `schema` (print the formal schema) · `selftest` · `--json` |
