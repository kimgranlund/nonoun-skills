...
- **A routing-eval corpus** (`evals/routing-corpus.json`) — trigger + adversarial phrases scored before the
  ...
  route to layout-decomposer (UI structure, not diagrams) and brand-forge (visual taste).
...
- **A worked CREATE + GRADE example per family** — one real diagram taken end-to-end on both axes, as a calibration
  reference for the rubric (the analog of layout-decomposer's per-archetype worked decompositions).
...
- **Cross-link the seam:** layout-decomposer (UI structure) hands a flow/state that wants a diagram here;
