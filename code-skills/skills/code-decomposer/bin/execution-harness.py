#!/usr/bin/env python3
"""execution-harness.py — the code-decomposer EXECUTION gate. Self-contained (stdlib only).

The EXECUTION axis (B1 parse/compile · B2 types/lint · B3 test) is mechanizable, but the toolchain
is project-specific — so this is a thin ADAPTER, not a bundled compiler. It reads a tiny manifest of
per-phase commands, runs each one whose tool is actually on PATH, and normalizes the verdicts into a
report card. Like the mermaid render-check, the static logic (manifest parse + verdict normalize) is
proved by `selftest` with zero external deps; the live run fires only where the real tools exist (a
missing tool is a SKIP, not a failure).

Manifest (JSON) — each phase optional; `gate` defaults by phase (compile/typecheck/test gate; lint/
mutation advisory):
  {
    "language": "python",
    "gates": {
  ...
  python3 bin/execution-harness.py selftest
  python3 bin/execution-harness.py template
  python3 bin/execution-harness.py <manifest.json> [--cwd DIR]
...
PHASES = ["compile", "typecheck", "lint", "test", "mutation"]   # canonical run order
GATE_BY_DEFAULT = {"compile": True, "typecheck": True, "lint": False, "test": True, "mutation": False}
...
def selftest():
    ...
    specs = parse_manifest({"gates": {"test": {"cmd": "pytest"}, "compile": {"cmd": "cc"}}})
    if [s["phase"] for s in specs] != ["compile", "test"]:
        errs.append("parse_manifest order wrong: %s" % [s["phase"] for s in specs])
    if specs[0]["gate"] is not True:
        errs.append("compile should gate by default")
    # 3. parse_manifest rejects malformed
    for bad in ({"gates": {"frobnicate": {"cmd": "x"}}}, {"gates": {"test": {}}}, {"nope": 1}):
        try:
            parse_manifest(bad)
    ...
    ok = run_phase({"phase": "test", "cmd": "%s -c \"import sys; sys.exit(0)\"" % shlex.quote(sys.executable), "gate": True})
    if not ok["ran"] or ok["verdict"] != "pass":
        errs.append("run_phase pass path failed: %s" % ok)
    bad = run_phase({"phase": "test", "cmd": "%s -c \"import sys; sys.exit(5)\"" % shlex.quote(sys.executable), "gate": True})
...
def _print_card(results, cwd):
    print("EXECUTION report card%s" % (" (cwd=%s)" % cwd if cwd else ""))
...
def main(argv):
    if not argv or argv[0] == "selftest":
        errs = selftest()
        if errs:
            sys.stderr.write("execution-harness: FAIL (%d)\n" % len(errs))
            for e in errs:
                sys.stderr.write("  - %s\n" % e)
            return 1
        print("execution-harness: OK — manifest parse + verdict normalize + run/skip paths verified")
        return 0
    if argv[0] == "template":
    ...
    try:
        ...
        sys.stderr.write("execution-harness: bad manifest — %s\n" % e)
        return 2
    results = [run_phase(s, cwd) for s in specs]
    _print_card(results, cwd)
    summary = overall(results)
    if summary["gate_skips"]:
        print("  ⚠ %d gate(s) skipped (tool absent) — evidence incomplete: %s"
