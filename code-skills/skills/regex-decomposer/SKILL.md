...
description: >
  ...
  (nested quantifiers, overlapping alternation, quadratic .*). The "wrong language" defect routes to
  an adversarial counter-example hunt — strings the spec implies but the examples forgot. Backed by a
  gated rubric and a pattern-spec card {pattern, flags, engine, positives[], negatives[]}. Use when
  designing a regex, grading one, hardening against ReDoS, or recovering what it matches. NOT for
  surrounding code (code-decomposer) or SQL (query-decomposer).
---
...
- **Nested quantifiers and overlapping alternation are ReDoS until proven safe.** `(\w+)+`, `(a|a)*`,
  `.*.*` are catastrophic-backtracking smells. `bin/regex-check.py` flags them statically; clear the
  flag with an adversarial-input timing test or a rewrite (atomic groups / possessive / RE2), never
  by ignoring it.
...
| `bin/regex-check.py` | **mechanizes B1–B3** — reads a pattern-spec card, compiles the pattern (Python `re`), asserts every positive matches and every negative does NOT under the `full`/`partial` mode, and runs a static ReDoS-smell scan (nested quantifier · overlapping alternation · quadratic wildcard); `<spec.json>` · `selftest` |
