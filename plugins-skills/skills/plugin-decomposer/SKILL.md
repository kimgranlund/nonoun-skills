...
| `bin/plugin-check.py` | **mechanizes B1–B4** — reads a `plugin.json` (+ optional marketplace entry) and flags MANIFEST_INVALID · BAD_NAME · BAD_VERSION · MARKETPLACE_MISMATCH · ILLEGAL_PATH (gates) + KITCHEN_SINK (advisory). `<plugin.json> [--marketplace M] [--entry NAME]` · `[--json]` machine-readable report (shared schema) · `template` · `selftest` |
