---
name: viz-2x2
description: Create shareable 2×2 matrix diagrams that illustrate strategic trade-offs across two independent axes — a self-contained HTML artifact with a matrix grid, summary cards, inline SVG charts, and a takeaway, light/dark mode. Use whenever the user asks to make a 2×2, comparison matrix, quadrant diagram, trade-off framework, or strategic positioning map — any visual placing four archetypes on two axes. Also trigger when the user describes four scenarios or modes and wants to visualize their relationships, even without saying "2×2". Triggers: "make a matrix", "show the trade-offs", "quadrant diagram", "four modes of X", "compare these four approaches". NOT for other diagram shapes — flowcharts, sequence/state/graph diagrams (mermaid-decomposer), Venn diagrams, decision trees, rankings, or comparison tables; NOT for UI-layout structure, regions, app shells, or wireframes (layout-decomposer); NOT for web-component design (component-decomposer); NOT for deriving brand positioning (brand-methodology / brand-evaluate).
---
